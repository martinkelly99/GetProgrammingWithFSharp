...
// extract date and abs diff between open and close values
// sort desc and select fst
...
tail.[0]
let a = splitCommas tail.[0]
let theDate = a.[0]
let theDiff = abs (System.Double.Parse(a.[1]) - System.Double.Parse(a.[4]))

let newList = List.map (fun x -> splitCommas x) tail
let newList2 = List.map (fun x -> (x[0],x[1],x[2])) newList:list<_>

let YouGotTheAnswerCorrect() =
    let (theDate, theDifference) = 
        stockData.Tail
        |> List.map (fun x -> splitCommas x)
        |> List.map (fun x -> (x.[0], abs (System.Double.Parse(x.[1]) - System.Double.Parse(x.[4]))))
